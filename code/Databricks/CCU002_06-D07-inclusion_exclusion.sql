...
-- MAGIC %md # ccu002_06-D07-inclusion_exclusion
-- MAGIC  
...
-- MAGIC %md
-- MAGIC Changes
-- MAGIC - age criterion to >= 18 (instead of 12)
-- MAGIC - project prefixes
...
-- MAGIC gdppr_data = 'dars_nic_391419_j3w9t_collab.ccu002_06_gdppr_dars_nic_391419_j3w9t'
-- MAGIC skinny_data = 'dars_nic_391419_j3w9t_collab.ccu002_06_skinny_patient'
-- MAGIC death_data = 'dars_nic_391419_j3w9t_collab.ccu002_06_deaths_dars_nic_391419_j3w9t'
-- MAGIC sgss_data = 'dars_nic_391419_j3w9t_collab.ccu002_06_sgss_dars_nic_391419_j3w9t'
-- MAGIC
-- MAGIC #Other data inputs
-- MAGIC conflicting_quality_assurance = 'dars_nic_391419_j3w9t_collab.ccu002_06_conflictingpatients'
-- MAGIC
-- MAGIC #Date parameters
-- MAGIC index_date = '2020-12-08'
-- MAGIC
-- MAGIC #Final table name
-- MAGIC final_table_1 = 'ccu002_06_included_patients'
...
-- MAGIC %py
-- MAGIC
-- MAGIC spark.sql(f"""create or replace temp view skinny as
-- MAGIC SELECT *
-- MAGIC FROM {skinny_data}""")
...
---%md %run "/Workspaces/dars_nic_391419_j3w9t_collab/ccu002_06c/Data_Curation/in_progress/CCU002-D03-quality_assurance_parameters_new"
...
create or replace temp view skinny_withQA as

SELECT t1.*
FROM skinny t1
LEFT JOIN dars_nic_391419_j3w9t_collab.ccu002_06_conflictingpatients t2 ---those who didnt meet QA criteria, from the previous notebook
...
-- MAGIC spark.sql(F"""create or replace temp view skinny_withQA2 as 
-- MAGIC SELECT *
-- MAGIC FROM skinny_withQA
...
-- MAGIC #Known sex and age 18 and over -- people who have not died before 8th Dec 2020 and who have SEX==1/2 and who are 18+
-- MAGIC spark.sql(F"""create or replace temp view patientinclusion AS 
-- MAGIC
-- MAGIC SELECT *
-- MAGIC FROM skinny_withQA2 
...
-- MAGIC spark.sql(F"""create or replace temp view positive_test AS   
-- MAGIC
...
create or replace temp view penultimate AS   
...
(
SELECT t1.*
FROM patientinclusion t1
LEFT JOIN  

(
SELECT * FROM positive_test --positive COVID test
...
-- MAGIC %sql 
-- MAGIC describe extended dars_nic_391419_j3w9t_collab.ccu002_06_included_patients
