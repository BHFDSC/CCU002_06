...
## =============================================================================
fit_get_data_surv <- function(interacting_feat, subgrp, fml, prev_vac_str, vac_str, covars,  agegp, event, survival_data, riskp_cuts){
  ...
  data.table::fwrite(tbl_event_count, 
                     paste0("tbl_event_count/tbl_event_count_", interacting_feat, subgrp, "_", dose_str, "_", event, "_", agegp, "_prev", prev_vac_str, "_", vac_str,".csv"), row.names = FALSE)
  ...
  data.table::fwrite(redacted_tbl_event_count, 
                     paste0("tbl_event_count_redacted/redacted_tbl_event_count_" , interacting_feat, subgrp, "_", dose_str, "_", event, "_", agegp, "_prev", prev_vac_str, "_", vac_str,".csv"), row.names = FALSE)
  ...
  if(dose_str=="booster"){ 
    ind_any_zeroeventperiod <- any(  # don't collpase for booster if no events >125d post-booster
  ...
  # firstly trying time period compression and secondly changing to a ?core? covariate set
