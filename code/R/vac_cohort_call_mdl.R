...
## Author: Samantha Ip
## =============================================================================
get_vacc_res <- function(event, prev_vac_str, vac_str, agegp, cohort_vac, covars){
  # event <-"first_ICVT"; prev_vac_str <- "vac_pf"; vac_str <- "vac_pfmod"; agegp <- "all"; fml <-  "+ weeks + agegroup + sex"
  ...
  if (ind_SA){
    cat("ind_SA", as.character(ind_SA), "......\n")
  ...
  for(fml in c( "+ weeks + agegroup + sex")){
    source(file.path(scripts_dir,"vac_cohort_fit_model.R"), local=TRUE)
    fit_model_reducedcovariates(fml, covars, prev_vac_str, vac_str, agegp, event, survival_data)
  }
